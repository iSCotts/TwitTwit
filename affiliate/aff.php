...
$checkcountofaffiliatetable ="SELECT count(*) FROM ta_affiliate_request WHERE UserID = '$_SESSION[username]'";
$checkcountofaffiliatetablestatus = runQuery($checkcountofaffiliatetable);
 if($checkcountofaffiliatetablestatus[0][0] == 0){
		 header("Location:../user/affiliate_program.php");
 }else{ ?>
 ...
 $(function()
{
 ...
 <!-- Middle Content Area Start -->
					...
					<div class="innermid_mid">
						...
						<div class="subhead_main">
							<div class="subhead_main_head"><h1>Your Affiliate  Details</h1></div>
								...
								<?php
						$getaffdetails ="SELECT * FROM ta_affiliate_request WHERE UserID = '$_SESSION[username]'";
						...
						?>
							<form name="aff_frm" method="POST" action="aff.php" >
	...
	if(($_REQUEST['startdate'] != '')&&($_REQUEST['enddate'] != ''))
	{
			...
			."WHERE af.`UserID` = '".$_SESSION['username']."' $sqlWhere  GROUP BY us.`SubsID` "
