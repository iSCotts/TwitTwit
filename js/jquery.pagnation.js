  ...
  $.fn.quickpaginate = function( settings ){
  	...
  	var setNav = function(){
  		if ( settings.pager === null ){	
  			settings.pager = jQuery('<div style="float:left;" class="qc_pager"></div>');
  			items.eq( items.size() -1 ).after(settings.pager);
  		}
		
  		var pagerNav = $('<div style="float:left; padding:10px 0 0 0;" class="'+settings.prev_class+'"><a href="#">'+settings.prev_text+'</a></div><div style="float:left; padding:10px 0 0 0;" class="'+settings.next_class+'"><a href="#">'+settings.next_text+'</a></div>');
		
  		jQuery(settings.pager).append( pagerNav );
		
  		if ( settings.showcounter ){
  			var counter = '<span style="float:left;" class="'+settings.counter+'"><span class="'+settings.pagenumber+'"></span> / <span class="'+settings.totalnumber+'"></span></span>';
