...
function getkeyword($messageid = false,$lang=false) {
	...
	if ($messageid) {
		...
		$sql = "SELECT K.KeyId, K.Message,KU.Lang, K.since_id, K.max_id FROM ta_keyword_users KU
					...
					where C.Status='A' AND KU.Followers< KU.FollowCount AND KeywordStatus='A' group by  K.KeyId ";
...
function addkeywordUser($request, $keyid) {
	...
	$db->Close();
	if ($nos == 0) {
		$sql = "insert into ta_keyword_users(keyId,UserId,CampaignID,Lang,FollowCount,FollowType) values($keyid,{$request['id']},'{$request['CampaignID']}','{$request['lang']}','{$request['count']}','{$request['type']}')";
		executeQuery($sql);
		return true;
	} else {
		return false;
