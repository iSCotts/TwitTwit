...
class twitter{
	 ...
	 * @var string
	 */
	protected $user_agent='twitacc:twitacc2010@gmail.com';
	 ...
	 */
	protected $type='json';
	protected $referrer = "http://www.twitjix.com/index.php"; 
	 ...
	 * @var boolean
	 */
	protected $suppress_response_code = false;
...
*/
	 ...
	 * Returns the last 200 updates by default
	 ...
	 * @return string
	 */
	function userTimeline($id=false,$count=200,$since=false,$since_id=false,$page=false)
	{
		if( !in_array( $this->type, array( 'xml','json','rss','atom' ) ) )
		return false;
	...
	function friendscursor($count, $id= false, $page = false )
	{
		if( !in_array( $this->type, array( 'xml','json' ) ) )
		return false;
		//$request = ( $id ) ? 'http://api.twitter.com/1/statuses/friends/' . $id . '.' . $this->type . $qs ."?cursor=".$count : 'http://api.twitter.com/1/statuses/friends.' . $this->type . $qs ."?cursor=".$count;
		$request = ( $id ) ? 'http://twitter.com/statuses/friends/' . $id . '.' . $this->type . $qs ."?cursor=".$count: 'http://twitter.com/statuses/friends.' . $this->type . $qs ."?cursor=". $count;
		//echo $request;
		//exit;
		return $this->objectify( $this->process($request) );
	}
	 ...
	 * @return string
	 */
	function followUser( $screenname="", $notifications = false,$id="" )
	{
...
*/
	 ...
	 * @return string
	 */
	function directMessages( $since = false, $count = 200, $since_id = false, $page = false )
	{
		if( !in_array( $this->type, array( 'xml','json','rss','atom' ) ) )
		return false;
		...
		$request = 'http://twitter.com/direct_messages.' . $this->type ."?count=200". implode( '&', $qsparams );
	...
	function updateColors( $colors = array() )
	{
		...
		if( preg_match('/^(?:(?:[a-f\d]{3}){1,2})$/i', $cv) ) :
	...
	function ratelimit()
	{
		...
		return $this->objectify( $request );
	}
	function dkratelimit()
	{
		if( !in_array( $this->type, array( 'xml','json' ) ) )
		return false;
		//$request = $this->process('http://twitter.com/account/rate_limit_status.' . $this->type);
		//return get_object_vars($this->objectify( $request ));
	    $ch = curl_init();
	    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
	    curl_setopt($ch, CURLOPT_URL, 'http://twitter.com/account/rate_limit_status.' . $this->type);
	    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); 
	    curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
	    ...
	    curl_close($ch);
		$remaining_hits = 0;
		//$requestdet =  $this->objectToArray($this->objectify( $buffer ));
		print_r($buffer);
		//if (array_key_exists('remaining_hits', $requestdet)) $remaining_hits = $requestdet['remaining_hits'];
...
class summize extends twitter
{
	 ...
	 * @var string
	 */
	protected $stype='json';
	function __constructor($user,$pass){
		parent::__constructor($user,$pass);
	}
	
	function search( $terms=false,$rpp='',$lang='',$callback=false,$since_id='' )
	{
		if( !$terms )
		return false;
		$qs = array();
        ...
        //commented on march12
		//$request .= '?q=' . urlencode($terms);
		if ($rpp!='') {
			$terms .= '&rpp=' . $rpp;
		}
		if ($lang!='') {
		...
		if( $callback && $this->stype == 'atom' )
		$qs[] = '';
	  	$data=$request . '?' . implode('&',$qs) ;
	  	$res= $this->process($data);
	   	return  $this->objectify($res);
